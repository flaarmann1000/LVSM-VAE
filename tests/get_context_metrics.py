...
for INPUT_VIEWS in [2,4,8,16]:
    ...
    df.to_csv(f"/home/teampc/LVSM-VAE/notebooks/results/performance_metrics/context{INPUT_VIEWS}.csv", index=False)
    with open(f"/home/teampc/LVSM-VAE/notebooks/results/performance_metrics/context{INPUT_VIEWS}_res.txt", "w") as f:
        f.write(f"\npsn: {mean_psnr}\nssim: {mean_ssim}\nlpips: {mean_lpips}")
